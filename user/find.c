...
{
    // If the pattern starts with '^', anchor the search to the beginning.
    if(re[0] == '^')
...
find(const char *path, const char *target)
{
    ...
    // NEW: Handle the case where the starting path itself is a matching file.
    if (st.type == T_FILE) {
        // basename(path)
        ...
        // This matching logic (copied from T_FILE block below) must be run for the starting path.
        int is_match = 0;
        if (g_use_regex)
        ...
        close(fd); // Close and return if it's a file, as files have no contents to recurse into.
        return;
    }
    
    // --- DIRECTORY TRAVERSAL STARTS HERE ---
    
    // Check if the starting path is a directory before proceeding.
    if (st.type != T_DIR) {
        fprintf(2, "find: %s is not a directory and was not handled\n", path);
        close(fd);
        return;
    }
    ...
    // ... rest of directory reading logic ...
    ...
    while (read(fd, &de, sizeof(de)) == sizeof(de)) {
        ...
        // Check if the entry is a directory (T_DIR) or a regular file (T_FILE)
        ...
        if(entry_st.type == T_FILE){
            // T_FILE block check
            int is_match = 0;
            if (g_use_regex)
                is_match = match((char *)target, (char *)name); // Use 'name' (basename)
            else
                is_match = (strcmp((char *)name, (char *)target) == 0); // Use 'name' (basename)

            if (is_match) {
                if (has_exec) run_exec_on(buf); // Use 'buf' (full path) for exec
                else printf("%s\n", buf);
            }
        } else if (entry_st.type == T_DIR) {
